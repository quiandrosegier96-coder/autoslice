"""Bambu Studio adapter layered on the slicer-neutral core parser."""
...
class BambuParser(ThreeMFParser):
    ...
    def can_parse(self, container: ThreeMFContainer) -> DetectionResult:
        ...
        return detected if detected.slicer is SlicerType.BAMBU else DetectionResult(SlicerType.BAMBU, 0.0)

    def parse(self, container: ThreeMFContainer) -> Universal3MFDocument:
        detection = self.can_parse(container)
        if detection.confidence <= 0:
            raise ValueError("BambuParser cannot parse a package not detected as Bambu Studio.")
        document = enrich_prusaslicer_family(self._core.parse(container), container)
        return replace(document, source=replace(
            document.source, slicer=SlicerType.BAMBU, confidence=detection.confidence,
